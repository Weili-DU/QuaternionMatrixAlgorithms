...
1. **SIPI/McMaster/BSD 论文同源测试集与文件列表**
   - 用途：对齐论文表格与图例中的同一批图像
   - 建议获取：按论文数据源链接下载，并提供作者使用的具体文件名清单

2. **基线方法官方实现与固定参数**
   - 用途：复现 Table 2 的横向对比
   - 建议获取：作者代码仓或补充材料；若无公开，需用户提供参考实现

3. **完整四元数实现细节（MATLAB版优先）**
   - 用途：缩小与论文在 `beta1/r` 最优点上的偏差
   - 建议获取：联系作者或获取附录代码

在补齐上述文件前，可完成“趋势级复现”；要达到“数值级严格对齐”需补齐后继续。
